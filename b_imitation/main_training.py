...
class EnhancedRealtimeGameEnv(gym.Env):
    ...
    def _reset_internal_state(self):
        ...
        # [新增] 记录上次打印的 assign_flag，防止刷屏
        self.last_printed_assign_flag = None

        # [新增] 死亡奖励防止重复触发标志位
        self.rewarded_1p_dead = False
        self.rewarded_2p_dead = False
    ...
    def step(self, action):
        ...
        if isinstance(action, (list, np.ndarray)):
            ...
            move_idx, skill_vec = 0, np.zeros(9, dtype=int)
    ...
    def _apply_action_masks(self, action, obs):
        ...
        # 能量检查 (bp_energy 是最新的)
        if self.bp_energy == 0.0: skill[8] = 0  # 替身
        ...
        if safe_get(64) != 0.0: skill[8] = 0  # space
        if safe_get(65) != 0.0: skill[0] = 0  # j
        if safe_get(66) != 0.0: skill[3] = 0  # i
    ...
    def _calculate_reward(self, raw_ids):
        ...
        # [修改] 不再强制置零负值，允许回血奖励
        # if deltab < 0: deltab = 0.000000
        # if deltar < 0: deltar = 0.000000

        # 异常过滤
        valid_blood = True

        # [修改]
        # - 移除 "回血跳过" (self.bblood > prev) 的逻辑
        # - 修改异常检测为 abs() > 0.3，以防止游戏重置（如0血变满血）被算作巨大回血
        ...
        # --- 2. 能量奖励 ---
        delta_bp = self.bp_energy - self.prev_bp_energy
        delta_rp = self.rp_energy - self.prev_rp_energy

        # 己方能量
        if abs(delta_bp - 0.25) < 1e-6:
            step_reward_change += 0.10
            reward_reasons.append("我方能量+1 (+0.1)")
        elif abs(delta_bp - 0.50) < 1e-6:
            step_reward_change += 0.20
            reward_reasons.append("我方能量+2 (+0.2)")
        elif abs(delta_bp + 0.25) < 1e-6:
            step_reward_change -= 0.10
            reward_reasons.append("我方能量-1 (-0.1)")
        elif abs(delta_bp + 0.50) < 1e-6:
            step_reward_change -= 0.20
            reward_reasons.append("我方能量-2 (-0.2)")
        elif abs(delta_bp + 1.00) < 1e-6:
            step_reward_change -= 0.40
            reward_reasons.append("我方能量-4 (-0.4)")

        # 敌方能量
        if abs(delta_rp - 0.25) < 1e-6:
            step_reward_change -= 0.10
            reward_reasons.append("敌方能量+1 (-0.1)")
        elif abs(delta_rp + 0.25) < 1e-6:
            step_reward_change += 0.10
            reward_reasons.append("敌方能量-1 (+0.1)")
        elif abs(delta_rp - 0.50) < 1e-6:
            step_reward_change -= 0.20
            reward_reasons.append("敌方能量+2 (-0.2)")
        elif abs(delta_rp + 0.50) < 1e-6:
            step_reward_change += 0.20
            reward_reasons.append("敌方能量-2 (+0.2)")
        ...
        if raw_ids and len(raw_ids) >= 2:
            ...
            # 1P 死亡
            if id_1p == 181 and not self.rewarded_1p_dead:
                self.rewarded_1p_dead = True  # 锁住，防止后续帧重复奖励
                if self.assign_flag == 'right':  # 1P=Enemy
                    step_reward_change += 1.0
                    reward_reasons.append("敌方(1P)死亡 (+1.0)")
                else:  # 1P=Self
                    step_reward_change -= 1.0
                    reward_reasons.append("我方(1P)死亡 (-1.0)")

            # 2P 死亡
            if id_2p == 181 and not self.rewarded_2p_dead:
                self.rewarded_2p_dead = True  # 锁住
                if self.assign_flag == 'right':  # 2P=Self
                    step_reward_change -= 1.0
                    reward_reasons.append("我方(2P)死亡 (-1.0)")
                else:  # 2P=Enemy
                    step_reward_change += 1.0
                    reward_reasons.append("敌方(2P)死亡 (+1.0)")
    ...
    def _check_done(self, t, p1, p2, cnn_vecs, raw_ids):
        ...
        # [修改] 移除了基于 cnn_vecs 的 None 判定逻辑

        # 3. [新增] 死亡判定 (ID=181)
        # raw_ids 顺序对应 cnn_regions: 0->1p, 1->2p
        if raw_ids and len(raw_ids) >= 2:
            id_1p = raw_ids[0]
            id_2p = raw_ids[1]
        ...
        if is_new_round:
            ...
            # [修改] 翻译身份: assign_flag=='right' 意味着 1P(Left)=Enemy, 2P(Right)=Self
            if self.assign_flag == 'right':
                who = "ENEMY (1P)"
            else:
                who = "SELF (1P)"
...
if __name__ == "__main__":
    ...
    if os.path.exists(PPO_MODEL_PATH):
        ...
        try:
            ...
            print(f"【警告】模型加载失败: {e}")
            model = None  # 标记失败，后续处理
    else:
        model = None

    # 2. 如果没有现有模型 (或加载失败)，尝试加载模仿学习预训练模型 (ppo_imitation.zip)
    if model is None:
        # 假设模仿学习模型也在当前目录 (b_imitation)
        ...
        if os.path.exists(imit_model_path):
            ...
            print("【提示】将以此为起点进行强化学习微调 (Fine-tuning)")
            try:
    ...
    try:
        model.learn(total_timesteps=100000, callback=EpisodeTrackerCallback(metadata))
        metadata.save()
