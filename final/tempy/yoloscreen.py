...
last_save_time = 0  # 上次保存的时间戳

while True:
    # --- 获取原始截图（BGR） ---
    frame_bgr = grab_screen(GAME_REGION)
    raw_frame = frame_bgr.copy()  # 保存用的原图
    ...
    # 如果有低置信度目标 & 距离上次保存超过1秒 → 保存 "原图"
    now = time.time()
    if need_save and (now - last_save_time >= 1.0):
        save_path = os.path.join(SAVE_DIR, make_name())
        cv2.imwrite(save_path, raw_frame)  # 保存原始截图
        last_save_time = now
