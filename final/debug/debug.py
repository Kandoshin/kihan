# Modified by assistant to implement Discrete Move/Skill heads + long-press + action masks
# Based on user's original file: :contentReference[oaicite:1]{index=1}

import os
import json
import time
import random
import queue
import numpy as np
import cv2
import mss
import torch
import gymnasium as gym
import pyautogui
import keyboard
import torch.nn as nn
import torch.nn.functional as F
import pickle
from datetime import datetime

from gymnasium import spaces
from PIL import Image
from torchvision import transforms, models
from typing import Tuple

from stable_baselines3 import PPO
from stable_baselines3.common.vec_env import DummyVecEnv, SubprocVecEnv
from stable_baselines3.common.torch_layers import BaseFeaturesExtractor
from stable_baselines3.common.policies import ActorCriticPolicy

import multiprocessing as mp
import threading
from concurrent.futures import ThreadPoolExecutor

pyautogui.PAUSE = 0

# ============= 原有路径/超参 =============
YOLO_PATH = r"G:\god\yolov5\yolov5-7.0\yolov5-7.0"
MODEL_PATH = r"G:\god\yolov5\yolov5-7.0\yolov5-7.0\best.pt"
GAME_REGION = {"left": 0, "top": 46, "width": 945, "height": 535}
DEVICE = "cuda:0" if torch.cuda.is_available() else "cpu"
CONF_THRES, IOU_THRES = 0.8, 0.8
TARGET_CLASSES = {'p', 'b', 'r', 's'}

PPO_MODEL_PATH = r"G:\god\pycharm\PythonProject\test\final\ppo_realtime.zip"
TRAINING_METADATA_PATH = r"G:\god\pycharm\PythonProject\test\final\training_metadata.json"
# CNN 相关
img_size = 70
save_dir = r"G:\god\pycharm\PythonProject\test\cnn\models"
class_map_path = os.path.join(save_dir, "class_to_idx.json")

if os.path.exists(class_map_path):
    with open(class_map_path, "r", encoding="utf-8") as f:
        class_to_idx = json.load(f)
else:
    print(f"警告：找不到 {class_map_path}，使用默认映射")
    class_to_idx = {"none": 0}

idx_to_class = {v: k for k, v in class_to_idx.items()}
num_classes = len(class_to_idx)
cnn_weight_path = os.path.join(save_dir, "best.pth")


# ============= CNN 处理器 (原样保留，少量注释) =============
def init_cnn_model():
    """在当前进程中初始化 CNN 模型"""
    try:
        model = models.mobilenet_v3_small(weights=None)
        model.classifier[3] = nn.Linear(model.classifier[3].in_features, num_classes)

        if os.path.exists(cnn_weight_path):
            sd = torch.load(cnn_weight_path, map_location=DEVICE)
            model.load_state_dict(sd, strict=True)
            print(f"成功加载 CNN 权重: {cnn_weight_path}")
        else:
            print(f"警告：找不到 CNN 权重文件 {cnn_weight_path}")

        model.to(DEVICE).eval()

        embedding_layer = nn.Embedding(num_classes, 16).to(DEVICE)
        transform = transforms.Compose([
            transforms.Resize((img_size, img_size)),
            transforms.ToTensor(),
            transforms.Normalize([0.5], [0.5])
        ])
        return model, embedding_layer, transform
    except Exception as e:
        print(f"CNN 模型初始化失败: {e}")
        return None, None, None


class ThreadCNNProcessor:
    """使用线程池而非多进程来避免句柄问题"""

    def __init__(self, max_workers=2):
        self.max_workers = max_workers
        self.executor = None
        self.cnn_model = None
        self.embedding_layer = None
        self.transform = None
        self.last_energy_features = {"blue_energy": 0.0, "red_energy": 0.0}

    def start(self):
        """启动线程池并初始化模型"""
        try:
            self.cnn_model, self.embedding_layer, self.transform = init_cnn_model()
            if self.cnn_model is None:
                print("警告：CNN 模型初始化失败，将使用默认值")
                return False

            self.executor = ThreadPoolExecutor(max_workers=self.max_workers)
            print(f"CNN 处理器启动成功，使用 {self.max_workers} 个线程")
            return True
        except Exception as e:
            print(f"CNN 处理器启动失败: {e}")
            return False

    def stop(self):
        """关闭线程池"""
        if self.executor:
            self.executor.shutdown(wait=True)
            print("CNN 处理器已关闭")

    def convert_predictions_to_features(self, pred_ids, assign_flag=None):
        """转换预测结果为特征，根据 assign_flag 调整 blue/red 顺序"""
        try:
            cnn_vecs = []
            name_to_idx = {reg["name"]: i for i, reg in enumerate(cnn_regions)}

            # 根据 assign_flag 决定 blue/red 对应哪些区域
            if assign_flag == 'left':  # blue 在左 (1p blue, 2p red)
                player_order = ['1p', '2p']  # blue, red
                energy_order = ['1p_energy', '2p_energy']
            else:  # 'right' 或 None: blue 在右 (2p blue, 1p red)
                player_order = ['2p', '1p']
                energy_order = ['2p_energy', '1p_energy']

            # blue player (16 dim)
            pred_id = pred_ids[name_to_idx[player_order[0]]]
            label = idx_to_class.get(int(pred_id), "none")
            if label == "none":
                cnn_vecs.extend([0.0] * 16)
            else:
                vec = self.embedding_layer(torch.tensor([pred_id], device=DEVICE)).squeeze(0).detach().cpu().numpy()
                cnn_vecs.extend(vec.tolist())

            # red player (16 dim)
            pred_id = pred_ids[name_to_idx[player_order[1]]]
            label = idx_to_class.get(int(pred_id), "none")
            if label == "none":
                cnn_vecs.extend([0.0] * 16)
            else:
                vec = self.embedding_layer(torch.tensor([pred_id], device=DEVICE)).squeeze(0).detach().cpu().numpy()
                cnn_vecs.extend(vec.tolist())

            # summon 和 scroll (各 16 dim)
            for name in ["summon", "scroll"]:
                pred_id = pred_ids[name_to_idx[name]]
                label = idx_to_class.get(int(pred_id), "none")
                if label.isdigit():
                    val = int(label) / 60.0
                    cnn_vecs.extend([val] * 16)
                elif label == "none":
                    cnn_vecs.extend([0.0] * 16)
                else:
                    vec = self.embedding_layer(torch.tensor([pred_id], device=DEVICE)).squeeze(0).detach().cpu().numpy()
                    cnn_vecs.extend(vec.tolist())

            # stand, skill1, skill2 (各 1 dim)
            for name in ["stand", "skill1", "skill2"]:
                pred_id = pred_ids[name_to_idx[name]]
                label = idx_to_class.get(int(pred_id), "none")
                cnn_vecs.append(int(label) / 60.0 if label.isdigit() else 0.0)

            # blue_energy (1 dim)
            pred_id = pred_ids[name_to_idx[energy_order[0]]]
            label = idx_to_class.get(int(pred_id), "none")
            if label.isdigit():
                feature_value = int(label) / 4.0
                self.last_energy_features["blue_energy"] = feature_value
                cnn_vecs.append(feature_value)
            else:
                cnn_vecs.append(self.last_energy_features["blue_energy"])

            # red_energy (1 dim)
            pred_id = pred_ids[name_to_idx[energy_order[1]]]
            label = idx_to_class.get(int(pred_id), "none")
            if label.isdigit():
                feature_value = int(label) / 4.0
                self.last_energy_features["red_energy"] = feature_value
                cnn_vecs.append(feature_value)
            else:
                cnn_vecs.append(self.last_energy_features["red_energy"])

            # time (1 dim)
            pred_id = pred_ids[name_to_idx["time"]]
            label = idx_to_class.get(int(pred_id), "none")
            if label == "none":
                cnn_vecs.append(-1.0)
            elif label.isdigit():
                val = int(label)
                cnn_vecs.append(round(val / 60.0, 4))
            else:
                cnn_vecs.append(-1.0)

            return cnn_vecs
        except Exception as e:
            print(f"转换预测结果失败: {e}")
            return [0.0] * self.get_expected_dim()

    def get_expected_dim(self):
        """计算预期的特征维度"""
        dim = 0
        for reg in cnn_regions:
            name = reg["name"]
            if name in {"1p", "2p"}:
                dim += 16
            elif name in {"summon", "scroll"}:
                dim += 16
            elif name in {"stand", "skill1", "skill2"}:
                dim += 1
            elif name in {"1p_energy", "2p_energy"}:
                dim += 1
            elif name == "time":
                dim += 1
        return dim

    def predict(self, crop_images, assign_flag=None, timeout=2):
        """预测图像特征"""
        if self.cnn_model is None or self.executor is None:
            return [0.0] * self.get_expected_dim()

        try:
            future = self.executor.submit(self._predict_sync, crop_images, assign_flag)
            return future.result(timeout=timeout)
        except Exception as e:
            print(f"CNN 预测失败: {e}")
            return [0.0] * self.get_expected_dim()

    def _predict_sync(self, crop_images, assign_flag):
        """同步预测函数"""
        try:
            tensors = []
            for crop in crop_images:
                img_rgb = cv2.cvtColor(crop, cv2.COLOR_BGR2RGB)
                pil_img = Image.fromarray(img_rgb)
                tensors.append(self.transform(pil_img))

            if not tensors:
                return [0.0] * self.get_expected_dim()

            batch_tensor = torch.stack(tensors).to(DEVICE)
            with torch.no_grad():
                logits = self.cnn_model(batch_tensor)
                probs = torch.softmax(logits, dim=1)
                confs, preds = torch.max(probs, dim=1)
                # —— 存低置信度图 ——
                save_dir = r"G:\god\pycharm\PythonProject\test\data\images"
                os.makedirs(save_dir, exist_ok=True)
                for idx, (crop, conf) in enumerate(zip(crop_images, confs.cpu().numpy())):
                    if conf < 0.8:
                        name = f"{int(time.time() * 1000)}{random.randint(100, 999)}.png"
                        cv2.imwrite(os.path.join(save_dir, name), crop)

                preds = preds.cpu().numpy()
                confs = confs.cpu().numpy()

                adjusted_preds = []
                for i, (p, c) in enumerate(zip(preds, confs)):
                    if c < 0.6:
                        adjusted_preds.append(class_to_idx.get("none", 0))
                    else:
                        adjusted_preds.append(int(p))

                return self.convert_predictions_to_features(adjusted_preds, assign_flag)

        except Exception as e:
            print(f"同步预测失败: {e}")
            return [0.0] * self.get_expected_dim()


# YOLO 初始化
def init_yolo_model():
    """初始化 YOLO 模型"""
    try:
        if not os.path.exists(YOLO_PATH):
            print(f"警告：YOLO 路径不存在: {YOLO_PATH}")
            return None
        if not os.path.exists(MODEL_PATH):
            print(f"警告：模型文件不存在: {MODEL_PATH}")
            return None

        yolo = torch.hub.load(YOLO_PATH, 'custom', path=MODEL_PATH, source='local', device=DEVICE)
        yolo.conf, yolo.iou = CONF_THRES, IOU_THRES
        yolo.eval()
        print("YOLO 模型加载成功")
        return yolo
    except Exception as e:
        print(f"YOLO 模型加载失败: {e}")
        return None


# 血条区域
MONITOR_1P = {"left": 102, "top": 80, "width": 169, "height": 6}
MONITOR_2P = {"left": 675, "top": 80, "width": 169, "height": 6}
HSV_LOW = np.array([0, 0, 251])
HSV_HIGH = np.array([179, 255, 255])

# 屏幕/血条区域
cnn_regions = [
    {"left": 30, "top": 48, "width": 70, "height": 70, "name": "1p"},
    {"left": 845, "top": 48, "width": 70, "height": 70, "name": "2p"},
    {"left": 824, "top": 151, "width": 56, "height": 56, "name": "summon"},
    {"left": 824, "top": 238, "width": 56, "height": 56, "name": "scroll"},
    {"left": 624, "top": 506, "width": 30, "height": 30, "name": "stand"},
    {"left": 726, "top": 505, "width": 30, "height": 30, "name": "skill1"},
    {"left": 738, "top": 400, "width": 30, "height": 30, "name": "skill2"},
    {"left": 96, "top": 92, "width": 60, "height": 16, "name": "1p_energy"},
    {"left": 790, "top": 92, "width": 60, "height": 16, "name": "2p_energy"},
    {"left": 428, "top": 83, "width": 88, "height": 66, "name": "time"},
]


def normalize_tuple(x, y, w, h, W, H):
    return (round(x / W, 4), round(y / H, 4), round(w / W, 4), round(h / H, 4))


def get_health(crop):
    """获取血量百分比（6位小数）"""
    try:
        hsv = cv2.cvtColor(crop, cv2.COLOR_BGR2HSV)
        mask = cv2.inRange(hsv, HSV_LOW, HSV_HIGH)
        row1 = int(mask[0].sum() // 255)
        row6 = int(mask[5].sum() // 255)
        if row1 > 82:
            return round(row1 / 164, 6)
        return round(row6 / 164, 6)
    except Exception as e:
        print(f"获取血量失败: {e}")
        return 1.00


# ============= 新的动作定义（与说明一致） =============
# Move Head (Discrete 9): 0 No-op, 1 W,2 S,3 A,4 D,5 WA,6 WD,7 AS,8 SD
MOVE_MAP = {
    0: set(),            # No-op
    1: {'w'},
    2: {'s'},
    3: {'a'},
    4: {'d'},
    5: {'w', 'a'},       # 左上 WA
    6: {'w', 'd'},       # 右上 WD
    7: {'a', 's'},       # 左下 AS
    8: {'s', 'd'},       # 右下 SD
}

# Skill Head (Discrete 10): 0 No-op, 1 J,2 K,3 L,4 I,5 U,6 O,7 Q,8 E,9 space
SKILL_MAP = {
    0: None,
    1: 'j',
    2: 'k',
    3: 'l',
    4: 'i',
    5: 'u',
    6: 'o',
    7: 'q',
    8: 'e',
    9: 'space'
}

KEYS = ['w', 's', 'a', 'd', 'j', 'k', 'l', 'i', 'u', 'o', 'q', 'e', 'space']

YOLO_DIM = len(TARGET_CLASSES) * 4 * 8
BASE_DIM = 2

CNN_DIM = 0
for reg in cnn_regions:
    name = reg["name"]
    if name in {"1p", "2p"}:
        CNN_DIM += 16
    elif name in {"summon", "scroll"}:
        CNN_DIM += 16
    elif name in {"stand", "skill1", "skill2"}:
        CNN_DIM += 1
    elif name in {"1p_energy", "2p_energy"}:
        CNN_DIM += 1
    elif name == "time":
        CNN_DIM += 1

OBS_DIM = BASE_DIM + YOLO_DIM + CNN_DIM
print(f"观测维度: BASE_DIM={BASE_DIM}, YOLO_DIM={YOLO_DIM}, CNN_DIM={CNN_DIM}, 总计={OBS_DIM}")

_global_cnn_processor = None


def get_global_cnn_processor():
    """获取全局 CNN 处理器"""
    global _global_cnn_processor
    if _global_cnn_processor is None:
        _global_cnn_processor = ThreadCNNProcessor()
        _global_cnn_processor.start()
    return _global_cnn_processor


class EnhancedRealtimeGameEnv(gym.Env):
    """增强的实时游戏环境，支持多种训练模式"""

    def __init__(self, env_id: int = 0):
        super().__init__()
        self.env_id = env_id
        # ========== 改动：统一为 MultiDiscrete([9, 2, 2, 2, 2, 2, 2, 2, 2, 2]) ==========
        self.action_space = spaces.MultiDiscrete([9, 2, 2, 2, 2, 2, 2, 2, 2, 2])

        self.observation_space = spaces.Box(low=-1.0, high=1.0, shape=(OBS_DIM,), dtype=np.float32)
        # 初始化屏幕捕获
        try:
            self.sct = mss.mss()
        except Exception as e:
            print(f"屏幕捕获初始化失败: {e}")
            self.sct = None

        # 初始化模型
        self.yolo_model = init_yolo_model()
        self.cnn_processor = get_global_cnn_processor()

        # 状态变量
        self.start_counter = 0
        self.end_counter_time = 0
        self.end_counter_p1 = 0
        self.end_counter_p2 = 0
        self.bblood = 1.00
        self.rblood = 1.00
        self.assign_flag = None
        self.init_bblood = 1.00
        self.init_rblood = 1.00
        self._latest_boxes = {}
        # 上一帧血量（用于计算差值）
        self.prev_bblood = self.bblood
        self.prev_rblood = self.rblood
        self.bp_energy = 0.0
        self.rp_energy = 0.0
        self.prev_bp_energy = 0.0
        self.prev_rp_energy = 0.0

        # 为 long-press 保存当前按下的键集合（字符串键）
        self.currently_pressed_keys = set()

        # 上一动作（两个头）
        self.prev_action = (0, np.zeros(9, dtype=int))

        # 累积奖励
        self.cumulative_reward = 0.00

        # FPS 控制
        self.last_step_time = 0
        self.min_step_interval = 0.01

        print(f"[Env {env_id}] 初始化完成")
        self._game_started = False
        self._episode_done = False
        # 新增并发控制
        self._key_control_enabled = True  # AI是否允许按键
        self._step_count = 0  # 用来在step里周期性检查
        self.last_output_time = time.time()  # 最后一次控制台输出的时间

    def log(self, *args, **kwargs):
        """统一打印，并刷新最后一次输出时间"""
        if self.env_id == 0:  # 仅主环境
            print(*args, **kwargs)
            self.last_output_time = time.time()

    def _get_obs(self) -> Tuple[np.ndarray, float, float, float, set]:
        """获取观测数据"""

        # FPS 控制
        now = time.time()
        elapsed = now - self.last_step_time
        if elapsed < self.min_step_interval:
            time.sleep(self.min_step_interval - elapsed)
        self.last_step_time = time.time()

        if self.sct is None:
            return self._get_default_obs()

        try:
            full_img = np.array(self.sct.grab(self.sct.monitors[0]))[..., :3]
        except Exception as e:
            print(f"屏幕捕获失败: {e}")
            return self._get_default_obs()

        # YOLO 检测
        yolo_flat, detected_classes = self._process_yolo(full_img)

        # 血量检测
        p1_blood, p2_blood = self._process_health(full_img)

        # CNN 处理
        cnn_vecs, time_val = self._process_cnn(full_img)

        # 更新血量分配
        self._update_blood_assignment(time_val, detected_classes, p1_blood, p2_blood)
        # 更新能量值（这两个在 convert_predictions_to_features 中更新）
        # 注意：cnn_vecs 的结构在 ThreadCNNProcessor.convert_predictions_to_features 中定义
        # blue_energy 在 cnn_vecs 的倒数第3， red_energy 倒数第2（和原实现一致）
        if cnn_vecs:
            try:
                # safe indexing if vector shorter than expected
                self.bp_energy = cnn_vecs[-3] if len(cnn_vecs) >= 3 else self.bp_energy
                self.rp_energy = cnn_vecs[-2] if len(cnn_vecs) >= 2 else self.rp_energy
            except Exception:
                pass

        # 构造观测
        base_obs = [self.rblood, self.bblood] + yolo_flat
        obs = np.array(base_obs + cnn_vecs, dtype=np.float32)

        return obs, time_val, self.rblood, self.bblood, detected_classes

    def _get_default_obs(self):
        """获取默认观测（当模型加载失败时）"""
        obs = np.zeros(OBS_DIM, dtype=np.float32)
        return obs, -1.0, 1.0, 1.0, set()

    def _process_yolo(self, full_img):
        """处理 YOLO 检测"""
        try:
            if self.yolo_model is None:
                return [0.0] * YOLO_DIM, set()

            g = GAME_REGION
            game_crop = full_img[g['top']:g['top'] + g['height'], g['left']:g['left'] + g['width']]
            # 1. 保存原始置信度，并临时设置一个极低的阈值以捕获所有检测结果
            original_conf = self.yolo_model.conf
            self.yolo_model.conf = 0.01  # 临时降低阈值
            results = self.yolo_model(cv2.cvtColor(game_crop, cv2.COLOR_BGR2RGB), size=320)
            self.yolo_model.conf = original_conf
            class_to_idx_yolo = {'p': 0, 's': 1, 'r': 2, 'b': 3}
            boxes = {k: [] for k in TARGET_CLASSES}
            detected_classes = set()

            # ---- 一旦有任何框置信度<0.6就存整图 ----
            for *_, conf, __ in results.pred[0]:
                if conf < 0.6:
                    save_dir = r"G:\god\pycharm\PythonProject\test\data\images"
                    os.makedirs(save_dir, exist_ok=True)
                    name = f"{int(time.time() * 1000)}{random.randint(100, 999)}.png"
                    cv2.imwrite(os.path.join(save_dir, name), game_crop)
                    break  # 同帧只存一次

            for *xyxy, conf, cls in results.pred[0]:
                # 使用原始的CONF_THRES (0.8) 来过滤，确保送入模型的观测是高质量的
                if conf < CONF_THRES:
                    continue
                cls_name = self.yolo_model.names[int(cls)]
                if cls_name in boxes:
                    detected_classes.add(cls_name)
                    x1, y1, x2, y2 = map(float, xyxy)
                    x_c, y_c = (x1 + x2) / 2, (y1 + y2) / 2
                    norm_box = normalize_tuple(x_c, y_c, x2 - x1, y2 - y1, g['width'], g['height'])
                    one_hot = [0.0] * 4
                    one_hot[class_to_idx_yolo[cls_name]] = float(conf)
                    boxes[cls_name].append(list(norm_box) + one_hot)

            yolo_flat = []
            for cls in sorted(TARGET_CLASSES):
                cls_boxes = boxes[cls][:4]
                while len(cls_boxes) < 4:
                    cls_boxes.append([0.0] * 8)
                for b in cls_boxes:
                    yolo_flat.extend(b)

            self._latest_boxes = boxes
            return yolo_flat, detected_classes
        except Exception as e:
            print(f"YOLO 处理失败: {e}")
            return [0.0] * YOLO_DIM, set()

    def _process_health(self, full_img):
        """处理血量检测"""
        try:
            crop1 = full_img[MONITOR_1P['top']:MONITOR_1P['top'] + MONITOR_1P['height'],
                    MONITOR_1P['left']:MONITOR_1P['left'] + MONITOR_1P['width']]
            crop2 = full_img[MONITOR_2P['top']:MONITOR_2P['top'] + MONITOR_2P['height'],
                    MONITOR_2P['left']:MONITOR_2P['left'] + MONITOR_2P['width']]

            p1_blood = get_health(crop1)
            p2_blood = get_health(crop2)

            return p1_blood, p2_blood
        except Exception as e:
            print(f"血量检测失败: {e}")
            return 1.00, 1.00

    def _process_cnn(self, full_img):
        """处理 CNN 特征提取"""
        try:
            crops = []
            for reg in cnn_regions:
                crop = full_img[reg["top"]:reg["top"] + reg["height"], reg["left"]:reg["left"] + reg["width"]]
                crops.append(crop)

            cnn_vecs = self.cnn_processor.predict(crops, self.assign_flag)
            time_val = cnn_vecs[-1] if cnn_vecs else -1.0

            return cnn_vecs, time_val
        except Exception as e:
            print(f"CNN 处理失败: {e}")
            return [0.0] * CNN_DIM, -1.0

    def _update_blood_assignment(self, time_val, detected_classes, p1_blood, p2_blood):
        """更新血量分配：根据 b 的横坐标中点决定蓝方在左还是右"""
        if time_val >= 0.999:
            self.init_bblood = self.bblood
            self.init_rblood = self.rblood

            b_boxes = []
            for cls, boxes in self._latest_boxes.items():
                if cls == 'b':
                    b_boxes.extend(boxes)

            if b_boxes:
                avg_x = sum(b[0] for b in b_boxes) / len(b_boxes)
                pixel_x = avg_x * GAME_REGION['width']
                self.assign_flag = 'right' if pixel_x > 415 else 'left'
                if self.env_id == 0:
                    print(f"【判定】b 在 {'右' if self.assign_flag == 'right' else '左'}半屏")
                    self.last_output_time = time.time()
            else:
                self.assign_flag = 'right'

        if self.assign_flag == 'right':
            self.bblood = p2_blood
            self.rblood = p1_blood
        else:
            self.bblood = p1_blood
            self.rblood = p2_blood

    def reset(self, *, seed=None, options=None):
        """重置环境：每次都强制等到下一轮倒计时 1.0"""
        super().reset(seed=seed)
        self._reset_state()
        self._episode_done = False

        # 关键：同步重置初始血量
        self.init_bblood = 1.00
        self.init_rblood = 1.00
        self.last_output_time = time.time()

        self.start_counter = 0
        while True:
            obs, t, _, _, _ = self._get_obs()

            if abs(t - 1.0) < 0.01:
                self.start_counter += 1
                self.start_counter += 1
            else:
                self.start_counter = 0

            if self.start_counter >= 5:
                if self.env_id == 0:
                    print("【提示】新一轮游戏开始")
                    self.last_output_time = time.time()

                self.end_counter_time = self.end_counter_p1 = self.end_counter_p2 = 0
                # 新增：如果及时开局，重置结束时间
                self.game_end_time = None
                return obs, {}

            time.sleep(0.01)

    def _reset_state(self):
        """重置状态变量"""
        self.start_counter = 0
        self.end_counter_time = 0
        self.end_counter_p1 = 0
        self.end_counter_p2 = 0
        self.bblood = 1.00
        self.rblood = 1.00
        self.assign_flag = None
        # 重置上一帧血量和奖励
        self.prev_bblood = self.bblood
        self.prev_rblood = self.rblood
        self.bp_energy = 0.0
        self.rp_energy = 0.0
        self.prev_bp_energy = 0.0
        self.prev_rp_energy = 0.0
        self.cumulative_reward = 0.00

        # 重置自动继续相关变量
        self.game_end_time = None

        # 重置 pressed keys
        self.currently_pressed_keys = set()
        self.prev_action = np.array([0, 0], dtype=int)

    def step(self, action):
        """执行一步"""

        # Accept different action shapes (from vectorized envs etc.)
        # Ensure action becomes a 1D int array [move, skill]
        # MultiDiscrete(10): [move, skill1, skill2, ..., skill9]
        if isinstance(action, (list, np.ndarray)):
            move_idx = int(action[0])
            skill_vec = np.array(action[1:], dtype=int).flatten()
        else:
            move_idx = 0
            skill_vec = np.zeros(9, dtype=int)
        action = np.concatenate([[move_idx], skill_vec])

        # 如果本局已结束，直接返回空奖励并保持终止状态
        if self._episode_done:
            obs = self._get_obs()[0]
            return obs, 0.00, True, False, {}

        obs, t, p1, p2, _ = self._get_obs()

        # 在执行动作前，根据 obs（CNN 输出）应用动作掩码规则
        masked_action = self._apply_action_masks(action, obs)

        # 仅主环境执行按键
        if self.env_id == 0 and self._key_control_enabled:  # 新增条件
            self._execute_actions(masked_action)

        # 计算奖励
        reward = self._calculate_reward(masked_action)
        self.prev_action = action

        # 检查游戏结束
        terminated = self._check_done(t, p1, p2)  # 真实终止
        truncated = False  # 无截断逻辑

        if terminated or truncated:
            self._episode_done = True

        return obs, reward, terminated, truncated, {}

    def _apply_action_masks(self, action, obs):
        """根据 CNN 特征对动作进行掩码（返回可能已被修改的动作）"""
        move_idx = int(action[0])
        skill_vec = np.array(action[1:], dtype=int)

        # ---- 示例：禁止某技能 ----
        # 替身(space, 最后一个技能)
        if self.bp_energy == 0.0:
            skill_vec[8] = 0

        # stand/skill1/skill2规则
        cnn_start = BASE_DIM + YOLO_DIM

        def safe_get(idx, default=0.0):
            if 0 <= idx + cnn_start < len(obs):
                return float(obs[cnn_start + idx])
            return default

        stand_val = safe_get(64)
        skill1_val = safe_get(65)
        skill2_val = safe_get(66)

        if stand_val != 0.0:
            skill_vec[8] = 0  # 禁止space
        if skill1_val != 0.0:
            skill_vec[0] = 0  # 禁止j
        if skill2_val != 0.0:
            skill_vec[3] = 0  # 禁止i

        # summon(q)/scroll(e)
        def block_all_equal(start_idx):
            vals = [safe_get(start_idx + k, 0.0) for k in range(16)]
            return all(abs(v - vals[0]) < 1e-6 for v in vals)

        if block_all_equal(32):
            skill_vec[6] = 0  # q 禁止
        if block_all_equal(48):
            skill_vec[7] = 0  # e 禁止

        return np.concatenate([[move_idx], skill_vec])

    def _execute_actions(self, action):
        """执行按键动作（长按行为）：仅按下所需新键，释放上步中不再需要的键"""
        if not self._key_control_enabled:
            return

        try:
            move_idx = int(action[0])
            skill_vec = np.array(action[1:], dtype=int)
            desired_keys = set(MOVE_MAP.get(move_idx, set()))

            # 多技能支持
            for i, v in enumerate(skill_vec,start=1):
                if v == 1 and i < len(SKILL_MAP) and i != 0:  # 0号代表No-op
                    k = SKILL_MAP[i]
                    if k is not None:
                        desired_keys.add(k)

            # 将 desired_keys 转换为与 KEYS 列表对齐的小写字符串（space 用 'space'）
            # current pressed keys tracked in self.currently_pressed_keys

            # keys to press: in desired_keys but not in currently_pressed_keys
            to_press = desired_keys - self.currently_pressed_keys
            # keys to release: in currently_pressed_keys but not in desired_keys
            to_release = self.currently_pressed_keys - desired_keys

            # 执行 release（先释放）
            for k in list(to_release):
                try:
                    pyautogui.keyUp(k)
                except Exception as e:
                    # ignore per-key errors
                    pass
                self.currently_pressed_keys.discard(k)

            # 执行按下
            for k in list(to_press):
                try:
                    pyautogui.keyDown(k)
                except Exception as e:
                    pass
                self.currently_pressed_keys.add(k)

        except Exception as e:
            print(f"按键操作异常: {e}")

    def _calculate_reward(self, action):
        """基于血量差值的奖励（使用上一帧 - 当前帧）"""
        # 计算差值：上一帧 - 当前帧
        deltab = (self.prev_bblood - self.bblood)
        deltar = (self.prev_rblood - self.rblood)

        # 强制置零：负值无效
        if deltab < 0:
            deltab = 0.00
        if deltar < 0:
            deltar = 0.00

        # 条件 1：双方血量都为 0 → 不更新
        if self.bblood == 0 and self.rblood == 0:
            # 滑动更新血量帧
            self.prev_bblood = self.bblood
            self.prev_rblood = self.rblood
            return self.cumulative_reward

        # 条件 2：回血 → 不更新
        if self.bblood > self.prev_bblood or self.rblood > self.prev_rblood:
            # 滑动更新血量帧
            self.prev_bblood = self.bblood
            self.prev_rblood = self.rblood
            return self.cumulative_reward

        # 新增条件：血量变化超过30% → 不更新（异常变化，忽略）
        if deltab > 0.3 or deltar > 0.3:
            # 滑动更新血量帧
            self.prev_bblood = self.bblood
            self.prev_rblood = self.rblood
            return self.cumulative_reward
        # --- 能量奖励 ---
        delta_bp_energy = self.bp_energy - self.prev_bp_energy
        delta_rp_energy = self.rp_energy - self.prev_rp_energy

        if abs(delta_bp_energy - 0.25) < 1e-6:
            self.cumulative_reward += 0.10
            if self.env_id == 0:
                print(f"能量奖励: 己方 +0.25 → +0.10")
        elif abs(delta_bp_energy - 0.50) < 1e-6:
            self.cumulative_reward += 0.20
            if self.env_id == 0:
                print(f"能量奖励: 己方 +0.50 → +0.20")
        elif abs(delta_bp_energy + 0.25) < 1e-6:
            self.cumulative_reward -= 0.10
            if self.env_id == 0:
                print(f"能量惩罚: 己方 -0.25 → -0.10")
        elif abs(delta_bp_energy + 0.50) < 1e-6:
            self.cumulative_reward -= 0.20
            if self.env_id == 0:
                print(f"能量惩罚: 己方 -0.50 → -0.20")
        elif abs(delta_bp_energy + 1.00) < 1e-6:
            self.cumulative_reward -= 0.40
            if self.env_id == 0:
                print(f"能量惩罚: 己方 -1.00 → -0.40")

        if abs(delta_rp_energy - 0.25) < 1e-6:
            self.cumulative_reward -= 0.10
            if self.env_id == 0:
                print(f"能量惩罚: 对方 +0.25 → -0.10")
        elif abs(delta_rp_energy + 0.25) < 1e-6:
            self.cumulative_reward += 0.10
            if self.env_id == 0:
                print(f"能量奖励: 对方 -0.25 → +0.10")
        elif abs(delta_rp_energy - 0.50) < 1e-6:
            self.cumulative_reward -= 0.20
            if self.env_id == 0:
                print(f"能量惩罚: 对方 +0.50 → -0.20")
        elif abs(delta_rp_energy + 0.50) < 1e-6:
            self.cumulative_reward += 0.20
            if self.env_id == 0:
                print(f"能量奖励: 对方 -0.50 → +0.20")

        # 更新奖励
        old_reward = self.cumulative_reward
        self.cumulative_reward += (deltar - deltab)

        # 打印奖励更新
        if abs(self.cumulative_reward - old_reward) > 1e-6 and self.env_id == 0:
            print(f"reward: {old_reward:.2f}→{self.cumulative_reward:.2f} (Δb={deltab:.2f}, Δr={deltar:.2f})")
            self.last_output_time = time.time()

        # 滑动更新血量帧
        self.prev_bblood = self.bblood
        self.prev_rblood = self.rblood
        self.prev_bp_energy = self.bp_energy
        self.prev_rp_energy = self.rp_energy

        return self.cumulative_reward

    def _check_done(self, t, p1, p2):
        """检查游戏是否结束"""
        if abs(t) < 1e-3:
            self.end_counter_time += 1
        else:
            self.end_counter_time = 0

        if p1 <= 0:
            self.end_counter_p1 += 1
        else:
            self.end_counter_p1 = 0

        if p2 <= 0:
            self.end_counter_p2 += 1
        else:
            self.end_counter_p2 = 0

        done = (self.end_counter_time >= 5 or
                self.end_counter_p1 >= 5 or
                self.end_counter_p2 >= 5)

        if done:
            if self.game_end_time is None:
                self.game_end_time = time.time()
            if self.env_id == 0:
                print("【环境】对局结束")
                print("【提示】本局结束，AI停止运行。请手动点击继续下一局。")
                self.last_output_time = time.time()
                self.release_all_keys()

        return done

    def release_all_keys(self):
        """释放所有按键"""
        if self.env_id == 0:
            try:
                for k in list(self.currently_pressed_keys):
                    pyautogui.keyUp(k)
                self.currently_pressed_keys.clear()
            except Exception as e:
                print(f"释放按键失败: {e}")

    def close(self):
        try:
            self.release_all_keys()
        except:
            pass
        if self.sct:
            try:
                self.sct.close()
            except:
                pass


# ============= 自定义策略相关（保留 CustomExtractor、CustomPPO，但使用 ActorCriticPolicy） =============
class CustomExtractor(BaseFeaturesExtractor):
    def __init__(self, observation_space: spaces.Box, features_dim: int = 256):
        super().__init__(observation_space, features_dim)
        input_dim = observation_space.shape[0]
        self.net = nn.Sequential(
            nn.Linear(input_dim, 256), nn.ReLU(),
            nn.Linear(256, 256), nn.ReLU(),
            nn.Linear(256, features_dim)
        )

        def init_skill_bias(module):
            if isinstance(module, nn.Linear) and module.bias is not None:
                nn.init.constant_(module.bias, -2.0)

        self.net.apply(init_skill_bias)
    def forward(self, obs):
        return self.net(obs)


policy_kwargs = dict(
    features_extractor_class=CustomExtractor,
    features_extractor_kwargs=dict(features_dim=256),
)


class CustomPPO(PPO):
    def train(self) -> None:
        self.policy.train()
        clip_range = self.clip_range(self._current_progress_remaining) if callable(
            self.clip_range) else self.clip_range
        for _ in range(self.n_epochs):
            for rollout_data in self.rollout_buffer.get(self.batch_size):
                values, log_prob, entropy = self.policy.evaluate_actions(
                    rollout_data.observations, rollout_data.actions
                )
                adv = rollout_data.advantages
                if self.normalize_advantage:
                    adv = (adv - adv.mean()) / (adv.std() + 1e-8)
                ratio = torch.exp(log_prob - rollout_data.old_log_prob)
                surr1 = ratio * adv
                surr2 = torch.clamp(ratio, 1.0 - clip_range, 1.0 + clip_range) * adv
                policy_loss = -torch.min(surr1, surr2).mean()
                value_loss = F.mse_loss(rollout_data.returns, values.flatten())
                entropy_loss = -torch.mean(entropy)

                loss = policy_loss + self.vf_coef * value_loss + self.ent_coef * entropy_loss
                self.policy.optimizer.zero_grad()
                loss.backward()
                if self.max_grad_norm is not None:
                    torch.nn.utils.clip_grad_norm_(self.policy.parameters(), self.max_grad_norm)
                self.policy.optimizer.step()


# ============= 训练元数据管理（原样保留） =============
class TrainingMetadata:
    """管理训练元数据,包括轮数、时间戳等"""

    def __init__(self, filepath: str):
        self.filepath = filepath
        self.data = self.load()

    def load(self) -> dict:
        """加载元数据"""
        if os.path.exists(self.filepath):
            try:
                with open(self.filepath, 'r', encoding='utf-8') as f:
                    data = json.load(f)
                print(f"✅ 加载训练元数据: 总轮数={data.get('total_episodes', 0)}, "
                      f"总步数={data.get('total_timesteps', 0)}")
                return data
            except Exception as e:
                print(f"⚠️ 加载元数据失败: {e}, 使用默认值")
        return {
            'total_episodes': 0,
            'total_timesteps': 0,
            'last_save_time': None,
            'training_history': []
        }

    def save(self):
        """保存元数据"""
        try:
            self.data['last_save_time'] = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
            with open(self.filepath, 'w', encoding='utf-8') as f:
                json.dump(self.data, f, indent=2, ensure_ascii=False)
            print(f"💾 元数据已保存: 总轮数={self.data['total_episodes']}")
        except Exception as e:
            print(f"❌ 保存元数据失败: {e}")

    def update_episode(self, reward: float, timesteps: int):
        """更新一轮训练数据"""
        self.data['total_episodes'] += 1
        self.data['total_timesteps'] += timesteps

        # 记录历史(保留最近100轮)
        history_entry = {
            'episode': self.data['total_episodes'],
            'reward': float(reward),
            'timesteps': timesteps,
            'timestamp': datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        }
        self.data['training_history'].append(history_entry)
        if len(self.data['training_history']) > 100:
            self.data['training_history'] = self.data['training_history'][-100:]

    def get_stats(self) -> dict:
        """获取统计信息"""
        return {
            'total_episodes': self.data['total_episodes'],
            'total_timesteps': self.data['total_timesteps'],
            'avg_reward': np.mean([h['reward'] for h in self.data['training_history']])
            if self.data['training_history'] else 0.00
        }


class EpisodeTrackerCallback:
    """追踪每个episode的训练数据"""

    def __init__(self, metadata: TrainingMetadata):
        self.metadata = metadata
        self.last_save_episode = 0

    def __call__(self, locals_dict, globals_dict):
        """回调函数"""
        # 注意：这是简化版本，PPO的实际episode追踪比较复杂
        # 每完成一定步数就更新一次
        if hasattr(self, '_step_count'):
            self._step_count += 1
        else:
            self._step_count = 1

        # 每1000步记录一次
        if self._step_count % 1000 == 0:
            reward = float(locals_dict.get("reward", 0.00))
            self.metadata.update_episode(reward=reward, timesteps=1000)

            # 每10轮保存一次
            if self.metadata.data['total_episodes'] - self.last_save_episode >= 10:
                self.metadata.save()
                self.last_save_episode = self.metadata.data['total_episodes']
                print(f"\n📈 已完成 {self.metadata.data['total_episodes']} 轮训练\n")

        return True

# ============= env 工厂函数 =============
def make_env(env_id):
    def _init():
        return EnhancedRealtimeGameEnv(env_id=env_id)

    return _init


# ============= 主入口 =============
if __name__ == "__main__":
    print("torch.cuda.is_available():", torch.cuda.is_available())
    print("DEVICE:", DEVICE)
    mp.set_start_method("spawn", force=True)

    # ========== 新增：初始化训练元数据管理器 ==========
    metadata = TrainingMetadata(TRAINING_METADATA_PATH)
    print(f"\n{'=' * 50}")
    print(f"📊 训练统计:")
    stats = metadata.get_stats()
    print(f"   - 历史总轮数: {stats['total_episodes']}")
    print(f"   - 历史总步数: {stats['total_timesteps']}")
    print(f"   - 近期平均奖励: {stats['avg_reward']:.2f}")
    print(f"{'=' * 50}\n")
    # =============================================

    num_envs = 4
    env_fns = [lambda _i=i: make_env(_i)() for i in range(num_envs)]
    env = DummyVecEnv(env_fns)

    # ========== 注意：将 policy 改为 ActorCriticPolicy（离散 MultiDiscrete 更适配） ==========
    if os.path.exists(PPO_MODEL_PATH):
        try:
            print("【提示】加载已有模型")
            model = CustomPPO.load(PPO_MODEL_PATH, env=env, custom_objects={"policy_class": ActorCriticPolicy})
        except Exception as e:
            print(f"【提示】加载失败，新建模型: {e}")
            model = CustomPPO(ActorCriticPolicy, env, policy_kwargs=policy_kwargs,
                              verbose=1,
                              n_steps=512, batch_size=256, learning_rate=3e-4)
    else:
        print("【提示】新建模型")
        model = CustomPPO(ActorCriticPolicy, env, policy_kwargs=policy_kwargs,
                          verbose=1,
                          n_steps=512, batch_size=256, learning_rate=3e-4)

    # ========== 新增：创建回调函数 ==========
    callback = EpisodeTrackerCallback(metadata)
    # =====================================

    try:
        print("【提示】开始训练…")
        # ========== 修改：使用回调函数 ==========
        model.learn(total_timesteps=100000, callback=callback)
        # =====================================

        # ========== 新增：训练完成后保存元数据 ==========
        metadata.save()
        # ==========================================

        print("【提示】训练完成")
    except KeyboardInterrupt:
        print("【提示】手动中断，准备保存…")
    finally:
        try:
            model.save(PPO_MODEL_PATH)
            # ========== 新增：保存元数据 ==========
            metadata.save()
            print(f"【提示】训练数据已保存,当前总轮数: {metadata.data['total_episodes']}")
            # ==================================
            print(f"【提示】模型已保存到 {PPO_MODEL_PATH}")
        except Exception as e:
            print(f"【错误】保存模型失败：{e}")
        env.close()
        print("【提示】清理完成，退出。")